...
const ACCOUNT_MAPPING: { [upId: string]: AccountMapping } = (ACCOUNT_MAPPING_LIST as AccountMappingList)
    .reduce((acc, map) => ({...acc, [map.upId]: map}), {});
...
export function upToYnabTransaction (upTransaction: UpTransaction): YnabTransaction {
    return {
        ...
        cleared: upTransaction.data.attributes.status === "HELD" ? YnabTransaction.ClearedEnum.Uncleared : YnabTransaction.ClearedEnum.Cleared,
